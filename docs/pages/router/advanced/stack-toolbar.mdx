...
The easiest way to add icons on Android is to use [Material Symbols](https://fonts.google.com/icons) via the [`@expo/material-symbols`](https://github.com/expo/expo/tree/main/packages/material-symbols) package, which ships pre-built XML vector drawables. Pass the imported drawable directly to the `icon` prop:

```tsx
<Stack.Toolbar.Button icon={require('@expo/material-symbols/star.xml')} onPress={() => {}} />
<Stack.Toolbar.Button icon={require('@expo/material-symbols/share.xml')} onPress={() => {}} />
<Stack.Toolbar.Menu icon={require('@expo/material-symbols/more_vert.xml')}>{/* ... */}</Stack.Toolbar.Menu>
```
...
<Stack.Toolbar.Button
  icon={
    process.env.EXPO_OS === 'ios'
      ? 'star.fill'
      : require('@expo/material-symbols/star.xml')
  }
  onPress={() => {}}
/>
```
